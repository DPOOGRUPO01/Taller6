...
public class InterfazLibreria extends JFrame
{
	...
	private ArrayList<Categoria> categoriasGeneradas;
	...
	{
		categoriasGeneradas=new ArrayList<>();
	...
	public void cargarArchivos(File archivo_categorias, File archivo_libros)
	{
		try
		{
			...
			this.categoriasGeneradas= libreria.darCategoriasNuevas();
			String mensaje = "";
			if (categoriasGeneradas.size()!=0) {
				for(int i =0;i<=categoriasGeneradas.size();i++)
				{
					mensaje=  mensaje+ "\n"+ i+") "+categoriasGeneradas.get(i).darNombre()+ "\n";
				}
	...
	{
		String autor = JOptionPane.showInputDialog(this, "Escriba el nombre de los autores de los libros que desea eliminar", "autor");
		if (autor != null)
		{
			ArrayList<Libro> librosQueNoSePudieronEliminar= new ArrayList<Libro>();
			librosQueNoSePudieronEliminar = libreria.eliminarLibros(autor);
			int cantidadDeLibrosEliminados= libreria.darLibrosEliminados().size();
			JOptionPane.showMessageDialog(this, "Numero de libros eliminados: "+cantidadDeLibrosEliminados, "Eliminados", JOptionPane.INFORMATION_MESSAGE);
			if(librosQueNoSePudieronEliminar.size()!=0)
				{
					String mensaje="";
					for(int i=0; i<=librosQueNoSePudieronEliminar.size(); i++)
					{
						mensaje = "" + librosQueNoSePudieronEliminar.get(i).darTitulo() + "\n";
					}
					
					JOptionPane.showMessageDialog(this, "Libros que no se pudieron eliminar: "+mensaje, "Eliminados", JOptionPane.INFORMATION_MESSAGE);
				}
			else
			{
				JOptionPane.showMessageDialog(this, "Todos los libros fueron eliminados " , "Eliminados", JOptionPane.INFORMATION_MESSAGE);
			}
		}
	}
	...
	{
		try {
			String aRenombrar = JOptionPane.showInputDialog(this, "Escriba el nombre de la categoria que quiere cambiar", "aRenombrar");
			String nuevoNombre = JOptionPane.showInputDialog(this, "Escriba el nuevo nombre por el que quiere renombrar la categoria anterior", "nuevoNombre");
			libreria.renombrarCategoria(aRenombrar,nuevoNombre);
			JOptionPane.showMessageDialog(this, "Operacion realizada con exito","Exito", JOptionPane.INFORMATION_MESSAGE);
			}
		catch (Exception e)
			{
			JOptionPane.showMessageDialog(this, e.getMessage(), "Error de categoria",
					JOptionPane.ERROR_MESSAGE);
			e.printStackTrace();
		}
	}
